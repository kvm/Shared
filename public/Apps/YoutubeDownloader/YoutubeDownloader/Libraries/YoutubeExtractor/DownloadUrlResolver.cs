...
namespace YoutubeExtractor {
    ...
    public static class DownloadUrlResolver
    {
        ...
        public static IEnumerable<VideoInfo> GetDownloadUrls(string videoUrl, bool decryptSignature = true)
        {
            ...
            bool isYoutubeUrl = true; // TryNormalizeYoutubeUrl(videoUrl, out videoUrl);
        ...
        private static bool IsValidTitleChar(char ch)
        {
            if (ch == ' ') return true;
        ...
        private static string GetVideoTitle(JObject json)
        {
            ...
            char[] ptitleStr = title.ToString().ToCharArray();

            for (int i = 0; i < ptitleStr.Length; i++)
                if (IsValidTitleChar(ptitleStr[i]))
                    MyStringBuilder.Append(ptitleStr[i]);

            return title == null ? String.Empty : MyStringBuilder.ToString();
        }
